...
[
  ...
  return (
    ...
    <div className='cursor-pointer shadow-sm my-3'>
        <span>
            {questionType.map((number)=>{
              return(
                <div className='flex items-center mx-2 text-lg gap-3 lg:mx-24 md:gap-12 py-5 ' key={number.id}>
                  <h1 className='md:text-xl font-semibold' key={number.id+1}>Question {number.id} :{number.question}</h1>
                  <h4 className='md:text-xl font-bold' key={number.id+2}>Ans={number.options[0].text}</h4>
                </div>
              )
            })}
        </span>
